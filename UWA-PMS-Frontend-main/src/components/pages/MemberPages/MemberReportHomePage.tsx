// src/pages/MemberPages/MemberReportHomePage.tsx
...
const MemberReportHomePage: React.FC = () => {
  ...
  // Load only THIS member's projects
  useEffect(() => {
    let alive = true;
    (async () => {
      try {
  ...
  // Load previously generated reports (for this member)
  const loadReports = async () => {
    try {
      ...
      const auth = getAuth();
      const res = await fetch(
        `${import.meta.env.VITE_BASE_URL}/api/member/pdf/list.php`,
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
  ...
  return (
    <div className='flex flex-col gap-6 pb-4'>
      ...
      <div className='flex flex-col border rounded-md'>
        <div className='grid grid-cols-4 font-bold text-sm bg-gray-100 p-2 border-b'>
        ...
        {loadingReports && (
          <>
            {[...Array(3)].map((_, idx) => (
              <div key={idx} className='grid grid-cols-4 text-xs p-2 border-b opacity-60'>
        ...
        {!loadingReports && filteredReports.length > 0 && filteredReports.map((r) => (
          <div key={r.id} className='grid grid-cols-4 text-xs p-2 border-b hover:bg-gray-50 transition'>
...
export default MemberReportHomePage;